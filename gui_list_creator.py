...
def load_api_key():
    ...
    try:
        ...
        API_KEY = config.get("Preferences", "API_KEY", fallback=None)
        if API_KEY:
            return True
        else:
            return False
    ...
    if not API_KEY:
        if not load_api_key():
            QMessageBox.critical(
                None,
                "API Key Error",
                f"YouTube API key not found or failed to load from {PREFERENCES_FILE}. Please configure it in the main Search tab.",
            )
            return False

    if not API_KEY:
        QMessageBox.critical(None, "API Key Error", "YouTube API key is missing.")
        return False
