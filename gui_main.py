...
class MainWindow(QMainWindow):
    ...
    def init_viewer_page_ui(self, viewer_page_layout):
        ...
        self.video_player.setUrl(QUrl("about:blank"))
    ...
    def load_api_key(self):
        ...
        if key:
            ...
            self.log_gui_event(
                "No API key found in preferences.", color=self.GUI_COLOR_WARNING
            )
    ...
    def handle_timestamp_click(self, url):
        ...
        if "youtube.com/watch" in url_str and "v=" in url_str and "t=" in url_str:
            try:
                ...
                embed_url = f"https://www.youtube.com/embed/{video_id}?start={start_seconds}&autoplay=1&modestbranding=1&rel=0"
                self.video_player.setUrl(QUrl(embed_url))

            except Exception as e:
                self.log_gui_event(
                    f"Error parsing YouTube timestamp or loading embed: {e}. Opening URL externally.",
                    color=self.GUI_COLOR_WARNING,
                )
...
if __name__ == "__main__":
    ...
    if not os.path.isdir(assets_folder):
        ...
        print(f"Error: Assets folder '{assets_folder}' not found. Exiting.")
